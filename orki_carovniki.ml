...
let rec strongest_wizard (wizards : wizard list) : wizard option =
  ...
  | [] -> None
  | s :: xs ->
  let strongest_xs = strongest_wizard xs in
	  (match strongest_xs with
	  | None -> Some w.ability
	  | Some v -> if Some w.ability > Some v.ability
					then Some w.ability
					else Some v.ability (*TUKAJ NE VEM KAKO PRIDEJO NOTR TE ABILITY. DEJ TO MAL POGLEJ. LP*)
	  )
